...
export default function Filter(props: FilterPropsType) {
    ...
    return (
        <> {category.page === 'Profile' && <div className="bg-red-300 text-slate-800">
            <select value={category.placesCategory} onChange={e => { category.setPlacesCategory?.(parseInt(e.target.value) as PLACE_CATEGORY) }}>
                <option value={PLACE_CATEGORY.MY_PLACES}>{PlaceCategoryMap[PLACE_CATEGORY.MY_PLACES]}</option>
                <option value={PLACE_CATEGORY.FAVORITE_PLACES}>{PlaceCategoryMap[PLACE_CATEGORY.FAVORITE_PLACES]}</option>
                <option value={PLACE_CATEGORY.LIKED_PLACES}>{PlaceCategoryMap[PLACE_CATEGORY.LIKED_PLACES]}</option>
            </select>
        </div>}
